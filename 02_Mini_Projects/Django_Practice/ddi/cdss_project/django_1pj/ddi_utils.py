...
# DeepDDI2 경로 추가
BASE_DIR = Path(__file__).resolve().parents[1]   # ...\cdss_project
PROJECT_ROOT =  Path(settings.BASE_DIR).parent   # ...\ddi
               
DATA_DIR  = PROJECT_ROOT / "deepddi2\data"
...
DRUG_INFO_FILE        = DATA_DIR / "Approved_drug_Information.txt"
ALL_APPROVED_FILE     = DATA_DIR / "all_approved_drug_input.txt"
PCA_PROFILE_FILE      = DATA_DIR / "drug_tanimoto_PCA50.csv"
BINARIZER_FILE        = DATA_DIR / "multilabelbinarizer.pkl"
INTERACTION_INFO_FILE = DATA_DIR / "Type_information" / "Interaction_information_model1.csv"

DDI_MODEL_PKL   = MODEL_DIR / "trained_rf_ovr.pkl"
DDI_MODEL_WEIGHT = None
sys.path.insert(0, str(PROJECT_ROOT))

print("[DEBUG] DATA_DIR:", DATA_DIR)
print("[DEBUG] DRUG_INFO_FILE exists?", os.path.exists(DRUG_INFO_FILE))
...
def load_drug_database():
    ...
    try:
        ...
        all_drug_input_file = str(ALL_APPROVED_FILE)
        db_to_smiles = {}
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_output_dir:
            ...
            DeepDDI.predict_DDI(
                ...
                df=pca_df,
                trained_model=str(DDI_MODEL_PKL),     # ✅ .pkl
                trained_weight=None,             # ✅ 미사용
                threshold=0.4,                   # 권장값(필요시 조절)
                binarizer_file=str(BINARIZER_FILE),
                model_type='model1'
            )
